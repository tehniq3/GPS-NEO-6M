 ...
 * niq_ro added 38 local time zones, from -12 to +14 with all real intermediate values
*/
...
format12 = EEPROM.read(adresa+1);
  ...
  if (timezone < 0) lcd.print("-");
  if (timezone != 0)
  lcd.print(abs(diferenta[timezone]));
  else lcd.print("     ");
  lcd.setCursor(6, 1);
  lcd.print("ver. 4.0");
  lcd.setCursor(0, 2);
...
if (abs(diferenta[timezone]) < 10)  lcd.print(" ");
  lcd.print("UTC");     
if (diferenta[timezone] > 0) lcd.print("+");
if (diferenta[timezone] < 0) lcd.print("-");
if (diferenta[timezone] != 0)
lcd.print(abs(diferenta[timezone]));
else lcd.print("     ");
